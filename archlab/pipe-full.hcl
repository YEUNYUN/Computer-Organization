...
#I have only implemented ICMPQ here. Small adjustment was to add the cmpq
#in decode bubble, which made no big difference if any. So I have worked on the
#to_binary_string.ys mainly.
...
word d_srcA = [
D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, ICMPQ  } : D_rA;
...
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, ICMPQ } : D_rB;
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) || (E_icode == ICMPQ && !e_Cnd) || 
	#SMALL ADITION HERE
	# Stalling at fetch while ret passes through pipeline
